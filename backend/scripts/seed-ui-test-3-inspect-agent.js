/**
 * Seed UI Test 3 — Inspect the Agent (INPUT = WorkflowItems only).
 * Usage: node scripts/seed-ui-test-3-inspect-agent.js
 */
...
const QUESTION =
  "What is 12 + 30? Use the calculator tool.";
...
(async () => {
  ...
  const id = uuidv4();
  const name = "UI Test 3 — Inspect the Agent";
  const description =
    "After Execute: open Basic AI Agent → INPUT must show WorkflowItem { question } only. Model/tools belong under Resources, not INPUT.";
  ...
  console.log("  INPUT  ✗ must NOT embed model/tools resource objects");
  console.log("  Resources (Parameters) → Model + Tools summary separately\n");
